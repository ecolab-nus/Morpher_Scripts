...
def main():
  ...
  DFG_GEN_KERNEL = DFG_GEN_HOME + '/applications/hycube_v3_design_app_test/gemm/gemm_no_unroll/'
  MAPPER_KERNEL = MAPPER_HOME + '/applications/hycube/gemm_no_unroll/'
  SIMULATOR_KERNEL =SIMULATOR_HOME + '/applications/gemm_no_unroll/'
...
##############################################################################################################################################
  ...
  print('\nGenerating DFG\n')
  os.system('./run_pass.sh gemm > dfg.log')
...
###############################################################################################################################################
  ...
  os.system('../../../build/src/cgra_xml_mapper -d gemm_INNERMOST_LN111_PartPred_DFG.xml -x 4 -y 4 -j hycube_original_mem_RC.json -i 12 -t HyCUBE_4REG  > map_right.log')
...
##############################################################################################################################################
  ...
  os.chdir(SIMULATOR_KERNEL)
  os.system('mv gemm_INNERMOST_LN111_mem_alloc.txt mem_alloc.txt')
  os.system('python3 ../../scripts/duplicate_config.py')

  for invocations in range(0,num_memory_traces) :
    data_file = 'memtraces/loop_gemm_INNERMOST_LN111_' + str(invocations) + '.txt'
