...
def main():
  ...
  MAPPER_KERNEL = MAPPER_HOME + '/applications/hycube8x8_full/array_add/'
  SIMULATOR_KERNEL =SIMULATOR_HOME + '/applications/hycube8x8_full/array_add/'
...
##############################################################################################################################################
  ...
  print('\nGenerating DFG\n')
  os.system('./run_pass.sh array_add 8 8192')
...
##############################################################################################################################################
  ...
  os.system('rm *.bin')  
  os.system('python ../../../update_mem_alloc.py ../../../json_arch/hycube_original_updatemem8x8.json array_add_INNERMOST_LN1_mem_alloc.txt 8192 8 hycube_original_mem.json')
  print('\nupdate memory allocation done!\n')
  os.system('../../../build/src/cgra_xml_mapper -d array_add_INNERMOST_LN1_PartPred_DFG.xml -x 8 -y 8 -j hycube_original_mem.json -t HyCUBE_4REG -i 4')
...
##############################################################################################################################################
  ...
  os.system('../../../src/build/hycube_simulator -c *.bin -d loop_array_add_INNERMOST_LN1_0.txt -a array_add_INNERMOST_LN1_mem_alloc.txt -m 65536 -x 8 -y 8 -t 2 > temp')
